...
//            table.setValueAt(aValue, row, column);
      ...
      setTable3(new JTable(dtm2));
      getTable3().setFont(new Font("맑은 고딕", Font.PLAIN, 30));
      getTable3().setRowHeight(64);
      
      getTable3().setAutoResizeMode(JTable.AUTO_RESIZE_OFF);
      getTable3().getColumnModel().getColumn(0).setPreferredWidth(100);
      getTable3().getColumnModel().getColumn(1).setPreferredWidth(290);
      getTable3().setBounds(800, 200, 400, 200);
      
      if(POSMainFrame.staticData != null){
         int sum = 0;
         for(int i = 0; i<POSMainFrame.staticData.length;i++){
            int j = Integer.parseInt(table.getValueAt(i, 4).toString());
            sum +=j;
         }
         getTable3().setValueAt(sum, 0, 1);
         
      }
      
      TableCellRenderer renderer = new MyTableCellRenderer();
      try {
         getTable3().setDefaultRenderer(Class.forName("java.lang.Object"), renderer);
